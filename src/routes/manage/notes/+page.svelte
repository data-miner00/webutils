...
<div class="flex h-full flex-col">
	<div class="flex items-center border border-b-0 border-solid border-gray-300 bg-gray-50">
		{#each notes as note}
			<button
				class={`relative flex w-[150px] items-center gap-2 border-l border-solid border-gray-300 px-3 py-2 text-sm first-of-type:border-l-0 ${currentId === note.id ? 'border-t-2 border-t-black bg-white after:absolute after:right-0 after:-bottom-1 after:left-0 after:h-1 after:bg-white' : 'bg-gray-50'}`}
	...
	<CodeEditor bind:language={currentLanguage} bind:value={content} class="flex-1" />
</div>
