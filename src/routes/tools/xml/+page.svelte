<script lang="ts">
    import { jsonToXml, exampleJson } from '$lib/core/xml';
	...
	let output = $derived(jsonToXml(input, { indentSize: Number(indentSize) }));
	function clearInput() {
		input = '';
	}
...
<div class="mb-4 flex h-screen">
	<section class="flex-1 pr-4">
		<header class="flex justify-between mb-6">
			<h1 class="text-xl font-bold block">JSON to XML Converter</h1>
		...
		<CodeEditor class="h-[500px]!" language="json" bind:value={input} />
	</section>
	...
	<section class="flex-1 pl-4">
		...
		<CodeEditor class="h-[500px]!" language="xml" value={output} readonly />
	</section>
</div>
