<script lang="ts">
	import { formatCss, exampleFormatCss, exampleFormatCss2 } from '$lib/core/format-css';
	...
	let output = $derived(formatCss(input, { indentSize: Number(indentSize) }));
	...
	function loadExample1() {
		input = exampleFormatCss;
	}

	function loadExample2() {
		input = exampleFormatCss2;
	}
</script>
...
<div class="mb-4 flex h-screen">
	<section class="flex-1 pr-4">
		<header class="flex justify-between mb-6">
			<h1 class="text-xl font-bold block">Format CSS</h1>
		...
		<CodeEditor class="h-[500px]!" language="css" bind:value={input} />
	</section>
	...
	<section class="flex-1 pl-4">
		...
		<CodeEditor class="h-[500px]!" language="css" value={output} readonly />
	</section>
</div>
